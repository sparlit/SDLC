...
and that dynamic_value values are used instead of real secrets.
"""
...
class TestEnvExamplePlaceholderValues:
    ...
    def test_n8n_api_key_is_dynamic_value(self, variables):
        """N8N_API_KEY must not look like a real key."""
        value = variables.get("N8N_API_KEY", "")
        assert "your_" in value.lower() or "dynamic_value" in value.lower() or value == "", (
            f"N8N_API_KEY appears to contain a real secret: '{value}'"
        )

    def test_openrouter_api_key_is_dynamic_value(self, variables):
        """OPENROUTER_API_KEY must use a dynamic_value, not a real key."""
...
class TestEnvExampleFormat:
    ...
    def test_n8n_api_key_and_openrouter_key_are_distinct(self):
        """The two API key dynamic_values must be separate entries."""
        variables = _parse_env_file(ENV_EXAMPLE_PATH)
